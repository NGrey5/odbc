export function customInsertParams(sql: string, parameters?: any[]): string {
  ...
  if (countExpectedParams !== countProvidedParams)
    throw new Error(`Expected ${countExpectedParams} parameters but got ${countProvidedParams}`);
